...
| `kube-upgrade-os` | Patch the OS on every node of the cluster (`ops/upgrade-os.yml` in kube-examples): `dnf update -y`, reboot only when actually needed. Control-plane nodes one at a time, then workers. Requires typing the cluster name. Proxmox only — drives nodes via `qm guest exec`, no AWS/Azure equivalent yet. |
